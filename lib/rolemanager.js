...
var RoleManager = {
    ...
    database.getDatabase(function (err, db) {
      ...
      app.rolemanager.retrieveRole({ _id: roleId }, function (err, role) {
        ...
        db.retrieve('user', { _id: userId }, { fields: 'roles' }, function (err, records) {
          ...
          roles.push(roleId);
          db.update('user', { _id: userId }, { 'roles': roles, invalidatePermissionCache: true }, function (err, records) {
            if (err) {
              return next(err);
            }
  ...
  unassignRole: function (roleId, userId, next) {
    database.getDatabase(function (err, db) {
      ...
      db.retrieve('user', { _id: userId }, { fields: 'roles' }, function (err, records) {
        ...
        db.update('user', { _id: userId }, { 'roles': roles, invalidatePermissionCache: true }, function (err) {
          if (err) {
            return next(err);
          }
  ...
  init: function (app) {
    ...
    // install default roles to the current tenant
    rest.post('/role/installdefault', function (req, res, next) {
      var tenant = usermanager.getCurrentUser().tenant;

      if (!tenant) {
        return next(new Error('Could not determine current tenant'));
      }

      installDefaultRoles(tenant, function (err) {
        if (err) {
          if (err.code != '11000') { // we can handle duplicate key errors as success
            return next(err);
          }

          res.statusCode = 200;
          return res.json({ success: true, message: 'Default roles exist' });
        }

        res.statusCode = 200;
        return res.json({ success: true, message: 'installed default roles' });
      });
    });
    ...
    fs.readdir(path.join(configuration.serverRoot, 'lib', 'role'), function (err, files) {
      async.each(
        ...
        function (file, callback) {
          ...
          db.update('role', {name: role.name}, { version: role.version, statment: role.statement }, function (err, rec) {
            if (err || !rec) {
              // Log error, but continue
