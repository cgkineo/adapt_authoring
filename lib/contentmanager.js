      ...
      async.eachSeries(plugins, function (plugin, outerCallback) {
                  ...
                  case 'menu':
                    ...
                    async.each(Object.keys(schemaPluginLocations.properties), function(key, nestedCallback) {
                      ...
                      if (pluginLocations.properties) {
                        ...
                        contentModelExtras[key].menuSettings = pluginLocations.properties;
                      }
                      nestedCallback();
                    }, function(err) {
        ...
        async.each(Object.keys(schemas), function(key, callback) {
          ...
          if (contentModelExtras[key]) {
            var props = {};
            async.each(Object.keys(contentModelExtras[key]), function(key2, callback2) {
              if(contentModelExtras[key][key2]) {
                value.properties[key2].properties = contentModelExtras[key][key2];
              }
              callback2();
            }, function() {
