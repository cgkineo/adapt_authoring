      ...
      async.eachSeries(plugins, function (plugin, outerCallback) {
                  ...
                  case 'menu':
                    ...
                    async.each(Object.keys(schemaPluginLocations.properties), function(key, nestedCallback) {
                      ...
                      if (pluginLocations.properties) {
                        ...
                        contentModelExtras[key][schema.name] = {
                         properties: pluginLocations.properties,
                         legend: schema.displayName,
                         targetAttribute: schema.targetAttribute,
                         type: "object"
                        };
                      }
                      nestedCallback();
        ...
        async.each(Object.keys(schemas), function(key, callback) {
          ...
          if (contentModelExtras[key]) {
            var props = {};
            for(var i = 0; i < Object.keys(contentModelExtras[key]).length; i++) {
              var PluginName = Object.keys(contentModelExtras[key])[i];
