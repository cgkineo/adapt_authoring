...
OutputPlugin.prototype.init = function() {
  var app = origin();
  var self = this;
  
  this.publishQueue = [];
  
  app.on('publish', function(courseId) {
    self.publishQueue.push(courseId);
  });
  
  app.on('published', function(courseId, success, data) {
    var i = self.publishQueue.indexOf(courseId);
    if(i > -1) self.publishQueue.splice(i, 1);
  });
};

OutputPlugin.prototype.isPublishing = function(courseId) {
  return this.publishQueue.indexOf(courseId) > -1;
};
...
OutputManager.prototype.getOutputPlugin = function (type, cb) {
  ...
  pluginManager.getPlugin('output', type, function (error, pluginInfo) {
    ...
    try {
      ...
      var plugin = new OutputPlugin(); // not sure we need to memoize
      self._outputTypes[type] = plugin;
      plugin.init();
